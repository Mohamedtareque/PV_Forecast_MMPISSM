"""
Physics-Residual Power Mamba (Spatial Wavelet V03) - V05 Hyper-Robust

Upgrades V02 with V05 Hyper-Robust Physics Layer:
- Irradiance Decomposition (Beam/Diffuse)
- Dynamic Logarithmic Wind Cooling
- Air Mass Dependent Spectral Efficiency
- Physics Isolation Principle
...
class DifferentiablePVLayer_V05(DifferentiablePVLayer_V4):
    """
    V05 Hyper-Robust:
    1. Irradiance Decomposition (Beam vs Diffuse)
    2. Dynamic Thermal Cooling (Logarithmic Wind)
    3. Spectral Efficiency (Air Mass dependent)
    """
    def __init__(self, idx_G, idx_Ta, idx_WS, idx_time_feats, idx_elev, **kwargs):
        # We need idx_elev for Air Mass calculation
        super().__init__(idx_G, idx_Ta, idx_WS, idx_time_feats, idx_elev=idx_elev, **kwargs)
        
        # 1. Irradiance Decomposition Params
        self.diffuse_frac_raw = nn.Parameter(torch.tensor(-2.0)) # Init low diffuse fraction

        # 2. Dynamic Thermal Cooling Params (Overrides U0, U1 linear model)
        # Log model: U_dynamic = U_base + U_wind * ln(1 + W + eps)
        self.U0_base_raw = nn.Parameter(torch.tensor(3.0))
        self.U_wind_raw = nn.Parameter(torch.tensor(1.5))
        
        # 3. Spectral Efficiency Params
        self.spec_coeff_raw = nn.Parameter(torch.tensor(0.0))
        
    def calculate_air_mass(self, elevation_degrees):
        """
        Kasten-Young Air Mass formulation.
        AM = 1 / (sin(alpha) + 0.50572 * (alpha + 6.07995)^-1.6364)
        """
        # Clamp elevation to avoid singularities near horizon
        ...
        # Clamp AM to reasonable values (e.g., <= 30)
    ...
    def forward(self, x_future, x_clr=None):
        ...
        # Apply Geometry/IAM primarily to Beam
        tilt_factor = self.geo_net(time_feats).squeeze(-1)
        ...
        # Effective Irradiance: Beam gets projected & IAM, Diffuse usually isotropic
        # Note: G_beam * tilt_factor approximates Plane-of-Array Beam
        # G_diff is assumed isotropic (simple addition), or could also scale. 
        # Standard: POA_Global = POA_Beam + POA_Diffuse + POA_GroundReflected
        # Simplifying: G_eff = (G_beam * tilt_factor * IAM) + G_diff
        G_eff = (G_beam * tilt_factor * IAM) + G_diff
        
        # 2. Dynamic Thermal Cooling (Logarithmic)
        U_base = F.softplus(self.U0_base_raw)
        U_wind = F.softplus(self.U_wind_raw)
        U_dynamic = U_base + U_wind * torch.log(1.0 + WS + self.eps)
        ...
        if self.idx_elev is not None:
            ...
            # eta_eff = eta_base * (1 + spec_coeff * (1 - AM))
            # Typically AM=1.0 is standard. 1 - AM becomes negative as AM increases.
            spec_coeff = self.spec_coeff_raw # Allow negative or positive
            spectral_factor = 1.0 + spec_coeff * (1.0 - AM)
        else:
            spectral_factor = 1.0
            
        eta_module_base = torch.sigmoid(self.eta_raw)
        eta_eff = eta_module_base * spectral_factor
        ...
        temp_factor = 1.0 + gamma * delta_T
        P_dc = eta_eff * P_stc * (G_eff / self.G_stc) * temp_factor
        P_dc = F.relu(P_dc)
...
class PhysicsResidualMamba_V05(nn.Module):
    """V05 Hyper-Robust: Advanced Physics with Decomposition/LogCooling/Spectral"""
    def __init__(self, configs):
        ...
        # Elevation for Air Mass
        try:
            idx_elev = configs.st07_future_cols.index('sun_elevation')
        except ValueError:
            print("Warning: sun_elevation not found for V05. Air Mass calc will use default/zeros.")
            idx_elev = None
...
def plot_fold_validation(fold_i, preds_dict, acts, dates_str, timestamps=None):
    ...
    colors = {'Vanilla Mamba': 'gray', 'Base Power Mamba': 'blue', 'Physics V02 (Basic)': 'green', 
              'Physics V03 (Geo)': 'orange', 'Physics V04 (Robust)': 'red', 
              'Physics V05 (Hyper-Robust)': 'purple', # New Color
...
# =============================================================================
# Convergence Plotting (New)
# =============================================================================

def plot_fold_convergence_comparison(fold_i, fold_histories_dict, save_path=None):
    """
    Comparison Plot for each fold to see which models converge fastest.
    Plots Training (Solid) and Validation (Dashed) loss for all models on one chart.
    """
    ...
    colors = {
        ...
        'Physics V04 (Robust)': 'red', 'Physics V05 (Hyper-Robust)': 'purple',
        'Physics Pure': 'gold', 'Smart Persistence': 'cyan', 'NWP Baseline': 'magenta'
    }
    ...
    plt.title(f'Fold {fold_i}: Model Convergence Comparison')
    plt.xlabel('Epochs')
...
def run_comparative_benchmark(df, n_splits=2, custom_config=None):
    print(">>> Running Spatial Wavelet Benchmark V03 (V05 Hyper-Robust)")
    ...
    model_classes = {
        ...
        'Physics V05 (Hyper-Robust)': PhysicsResidualMamba_V05,
        'Physics Pure': PhysicsResidualMamba_Pure,
    ...
    for fold_i, (train_loader, test_loader, _, dates) in enumerate(fold_gen):
        ...
        for name, model in models.items():
            ...
            for ep in range(epochs):
                ...
                train_loss = train_one_epoch_v02(model, train_loader, optimizers[name], device, loss_fn=loss_fns[name], warmup=warmup)
...
if __name__ == "__main__":
    print("Spatial Physics-Residual Mamba Wavelet V03")
    print("Features: V05 Hyper-Robust + DWT Decomposition")
