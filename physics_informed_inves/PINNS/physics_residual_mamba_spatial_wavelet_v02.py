"""
Physics-Residual Power Mamba (Spatial Wavelet V02)

Upgrades Spatial V02 by replacing Moving Average decomposition with 
Discrete Wavelet Transform (DWT) for better high-frequency transient capture.

Key Features:
1. Physics Isolation Principle (preserved from Spatial V02)
2. Wavelet Decomposition (db4) - zero phase lag, better cloud edge detection
3. Per-fold loss curve plotting
"""
...
import time
import pywt  # PyWavelets for DWT decomposition
...
class WaveletDecomp(nn.Module):
    """
    Multi-resolution Wavelet Decomposition Layer.
    Decomposes input into Approximation (Trend) and Detail (Residual).
    Uses interpolation to match original sequence length for downstream layers.
    """
    ...
    def forward(self, x):
        ...
        # Reshape for PyWavelets: (Batch * Channels, Length)
        x_flat = x.permute(0, 2, 1).contiguous().view(-1, L)
        
        # Perform Decomposition (Single Level) - on CPU for pywt
        x_np = x_flat.detach().cpu().numpy()
        
        # Handle NaN values
        x_np = np.nan_to_num(x_np, nan=0.0)
        
        # coeffs: [cA, cD] -> cA: Low Freq (Approx), cD: High Freq (Detail)
        ...
        # CRITICAL: Resizing to match original Length L
        # Wavelets downsample by ~2. Upsample to feed into Linear layers expecting size L.
        ...
        # Return Detail (Residual/Seasonal), Approximation (Trend)
...
class MMPISSM_Model_01(nn.Module):
    ...
    def __init__(self, configs, output_residual=False):
        ...
        self.wavelet_decomp = WaveletDecomp(wavelet='db4')  # Replaced series_decomp with Wavelet
    ...
    def forward(self, x, x_f):
        ...
        seasonal_init, trend_init = self.wavelet_decomp(x)  # Wavelet decomposition
...
# =============================================================================
# Per-Fold Loss Plotting
# =============================================================================

def plot_fold_loss_curves(fold_i, model_losses, save_path=None):
    """
    Plot training and validation loss curves for each model in a fold.
    ...
    """
    n_models = len(model_losses)
    fig, axes = plt.subplots(2, 4, figsize=(16, 8))
    axes = axes.flatten()
    
    for idx, (name, losses) in enumerate(model_losses.items()):
        if idx >= len(axes):
            break
        ax = axes[idx]
        
        if len(losses['train']) > 0:
            epochs = range(1, len(losses['train']) + 1)
            ax.plot(epochs, losses['train'], 'b-', label='Train', linewidth=1.5)
            if len(losses['val']) > 0:
                ax.plot(epochs, losses['val'], 'r--', label='Val', linewidth=1.5)
            ax.set_xlabel('Epoch')
            ...
            ax.text(0.5, 0.5, f'{name}\n(Baseline)', ha='center', va='center', fontsize=10)
            ax.set_xlim(0, 1)
            ax.set_ylim(0, 1)
    ...
    plt.suptitle(f'Fold {fold_i} Training Progress (Wavelet Decomposition)', fontsize=14)
    plt.tight_layout()
    
    filename = save_path or f"fold_{fold_i}_loss_curves.png"
...
# =============================================================================
# Benchmark Runner (with Per-Fold Loss Tracking)
# =============================================================================

def run_comparative_benchmark(df, n_splits=2, custom_config=None):
    """Run spatial-aware wavelet benchmark with Physics Isolation and Loss Tracking"""
    print(">>> Running Spatial Wavelet Benchmark (Target: K_PV, DWT Decomposition)")
    ...
    for fold_i, (train_loader, test_loader, _, dates) in enumerate(fold_gen):
        ...
        # Per-fold loss tracking for all models
        fold_model_losses = {name: {'train': [], 'val': []} for name in model_classes.keys()}
        ...
        for name, model in models.items():
            ...
            for ep in range(epochs):
                ...
                fold_model_losses[name]['train'].append(train_loss)
                
                # Validation loss per epoch
                model.eval()
                with torch.no_grad():
                    val_mets, _, _, _, _ = evaluate_v02(model, test_loader, device)
                    fold_model_losses[name]['val'].append(val_mets['mse_loss'])
                model.train()
        ...
        plot_fold_loss_curves(fold_i + 1, fold_model_losses)
...
if __name__ == "__main__":
    print("Spatial Physics-Residual Mamba Wavelet V02")
    print("Features: Physics Isolation + DWT Decomposition (db4)")
    print("Per-fold loss curve plotting enabled")
    c = SpatialMambaConfigs()
    print(f"St07 Indices: {c.st07_indices}")
    print(f"Wavelet: db4 (Single-level decomposition)")
