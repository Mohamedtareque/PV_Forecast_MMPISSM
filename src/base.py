...
from .utils import SinkhornDistance, SoftDTWBatch,PathDTWBatch,pairwise_distances, tdlgm_loss
...
# from .tdlgm.tdlgm import compute_loss as tdlgm_loss 
...
class Base(pl.LightningModule):
        ...
        if batch_idx==0:
            ...
            if self.count_epoch%int(max(self.trainer.max_epochs/100,1))==1:
                ...
                for i in range(batch['y'].shape[2]):
                    ...
                    y_hatt = y_hat[0] if isinstance(y_hat, tuple) else y_hat
                    pred =  y_hatt[0,:,i,idx].cpu().detach().numpy()
    ...
    def compute_loss(self,batch,y_hat):
        ...
        :meta private:
        """
        if self.loss_type == 'tdlgm_loss':
            if self.use_quantiles is False:

                initial_loss = self.loss(y_hat[0][:,:,:,0], batch['y'])
            else:
            ...
            x =  batch['x_num_past'].squeeze(dim=1).to(self.device) if len(batch['x_num_past'].shape) == 1 else batch['x_num_past'].to(self.device)
            idx_target = batch['idx_target'][0]

            if isinstance(idx_target, torch.Tensor):
                idx_target = idx_target.item()  # Convert tensor to Python int
                
            x_start = x[:,-1,idx_target].unsqueeze(1)
            x_start = x_start.view(x_start.shape[0], 1, -1)  # Ensure it has [batch, 1, features]
            y_persistence = x_start.repeat(1,self.future_steps,1)
            #y_persistence =  batch['y'].to(self.device).mean(axis=1).unsqueeze(1).repeat(1,self.future_steps,1) ##quello che fa meno errori e' la media
        ...
        elif self.loss_type=='tdlgm_loss':
            y_hat, gen_states, mean, R, time_states = y_hat

            loss = tdlgm_loss(batch['y'], y_hat[:,:,:,0], mean, R, gen_states, time_states)
